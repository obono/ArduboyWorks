...
bool MyArduboyV::nextFrame(void)
{
...
bool MyArduboyV::buttonDown(uint8_t buttons)
{
    return currentButtonState & ~lastButtonState & buttons;
}

bool MyArduboyV::buttonPressed(uint8_t buttons)
{
    return currentButtonState & buttons;
}


bool MyArduboyV::buttonUp(uint8_t buttons)
{
    return ~currentButtonState & lastButtonState & buttons;
}
...
void MyArduboyV::setTextColor(uint8_t color)
{
    setTextColor(color, (color == BLACK) ? WHITE : BLACK);
}

void MyArduboyV::setTextColor(uint8_t color, uint8_t bg)
{
    textcolor = color;
    textbg = bg;
}

size_t MyArduboyV::printEx(int16_t x, int16_t y, const char *p)
{
    setCursor(x, y);
    return print(p);
}

size_t MyArduboyV::printEx(int16_t x, int16_t y, const __FlashStringHelper *p)
{
    setCursor(x, y);
    return print(p);
}

size_t MyArduboyV::write(uint8_t c)
{
    if (c == '\n') {
        cursor_x += textSize * 6;
        cursor_y = HEIGHT - 1;
        ...
        cursor_y -= textSize * 6;
        if (textWrap && (cursor_y < textSize * 6 - 1)) write('\n');
    }
    return 1; // temporary
}

void MyArduboyV::myDrawChar(int16_t x, int16_t y, unsigned char c, uint8_t color, uint8_t bg, uint8_t size)
{
    bool draw_bg = bg != color;

    if (x >= WIDTH || y >= HEIGHT + 6 * size || x + 6 * size < 0 || y < 0) return;
    ...
    if (size == 1) {
        for (int8_t i = 0; i < 6; i++) {
            for (int8_t j = 0; j < 6; j++) {
                ...
                if (draw_fg || draw_bg) {
                    drawPixel(x + j, y - i, (draw_fg) ? color : bg);
                }
                ptn >>= 1;
            }
        }
        ...
        for (int8_t i = 0; i < 6; i++) {
            for (int8_t j = 0; j < 6; j++) {
                bool draw_fg = ptn & 0x1;
                if (draw_fg || draw_bg) {
                    fillRect(x + j * size, y - (i + 1) * size + 1, size, size, (draw_fg) ? color : bg);
                }
                ptn >>= 1;
            }
        }
...
void MyArduboyV::drawRect2(int16_t x, int16_t y, uint8_t w, int8_t h, uint8_t color)
{
...
void MyArduboyV::drawFastVLine2(int16_t x, int16_t y, int8_t h, uint8_t color)
{
    /*  Check parameters  */
    if (y < 0) {
        if (h <= -y) return;
...
void MyArduboyV::drawFastHLine2(int16_t x, int16_t y, uint8_t w, uint8_t color)
{
    /*  Check parameters  */
    if (x < 0) {
        if (w <= -x) return;
...
void MyArduboyV::fillRect2(int16_t x, int16_t y, uint8_t w, int8_t h, uint8_t color)
{
    /*  Check parameters  */
    if (x < 0) {
        if (w <= -x) return;
...
void MyArduboyV::fillBeltBlack(buffer_t *p, uint8_t d, uint8_t w)
{
    d = ~d;
    for (; w > 0; w--) {
        *p++ &= d;
...
void MyArduboyV::fillBeltWhite(buffer_t *p, uint8_t d, uint8_t w)
{
    for (; w > 0; w--) {
        *p++ |= d;
    }
...
bool MyArduboyV::isAudioEnabled(void)
{
    return myAudio.enabled();
}

void MyArduboyV::setAudioEnabled(bool on)
{
    if (on) {
        myAudio.on();
    } else {
...
void MyArduboyV::saveAudioOnOff(void)
{
    myAudio.saveOnOff();
}

void MyArduboyV::playScore2(const byte *score, uint8_t priority)
{
...
void MyArduboyV::stopScore2(void)
{
    pTunes->stopScore();
}
